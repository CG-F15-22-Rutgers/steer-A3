...
Util::Vector SocialForcesAgent::calcProximityForce(float dt)
{
	...
	//std::cout << " proxi finished" << std::endl;
	//std::cout << proximity_force << std::endl;
...
Vector SocialForcesAgent::calcGoalForce(Vector _goalDirection, float _dt)
{
	...
	goal_force = ((_goalDirection * PREFERED_SPEED - this->velocity()) / _dt)* AGENT_MASS;
	//std::cout << "preF " << std::endl;
	//std::cout << goal_force << std::endl;
	return goal_force;
}
...
Util::Vector SocialForcesAgent::calcAgentRepulsionForce(float dt)
{
    ...
    for(std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbour = _neighbors.begin();  neighbour != _neighbors.end(); neighbour++)
       {
		   if ((*neighbour)->isAgent())
		   {
			   //std::wcout << " found an agent nearby" << std::endl;
			   tmp_agent = dynamic_cast<SteerLib::AgentInterface *>(*neighbour);
		   }
            else
                continue; 
		   if( ( id() != tmp_agent->id()) && (tmp_agent->computePenetration(this->position(), this->radius()) > 0.000001))
            {
				//std::cout << "agent_R exist" << std::endl;
	...
	//std::cout << " agent_R " << std::endl;
	//std::cout << agent_repulsion_force << std::endl;
...
Util::Vector SocialForcesAgent::calcWallRepulsionForce(float dt)
{
   //std::cerr<<"<<<calcWallRepulsionForce>>> Please Implement my body\n";
	...
	for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbour = _neighbors.begin(); neighbour != _neighbors.end(); neighbour++)
	{
		if (!(*neighbour)->isAgent())
		{
			//std::wcout << " found an obs nearby" << std::endl;
			tmp_ob = dynamic_cast<SteerLib::ObstacleInterface *>(*neighbour);
		}
		else
			continue;
		...
		{
			//std::cout << "wall_R exist " << std::endl;
			...
			wall_repulsion_force = wall_repulsion_force + wall_normal * (radius() - min_stuff.first) * _SocialForcesParams.sf_body_force;//wall_repulsion_force_tmp
			//wall_repulsion_force_list.push_back(wall_repulsion_force_tmp); 

		}
	}
	//std::cout << " wall_R " << std::endl;
	//std::cout << wall_repulsion_force << std::endl;
